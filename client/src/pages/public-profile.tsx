...
import { Loader2, Mail, MapPin, Briefcase, Star, LayoutDashboard, Calendar, Award, ArrowLeft } from "lucide-react";
    ...
    if (error || !profile) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center gap-4">
                <h1 className="text-2xl font-bold">{t("public_profile.not_found")}</h1>
                <p className="text-muted-foreground">{t("public_profile.not_found_desc")}</p>
                <Button onClick={() => window.history.back()}>{t("public_profile.go_back")}</Button>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-background">
            ...
            <div className="bg-muted/30 border-b relative">
                <div className="container mx-auto px-4 py-8 md:py-12">
                    <Button variant="ghost" className="mb-6 -ml-2" onClick={() => window.history.back()}>
                        ...
                        <div className="flex-1 space-y-4">
                            <div>
                                ...
                                <p className="text-xl text-muted-foreground flex items-center gap-2 mt-1">
                                    ...
                                    {profile.title || t("public_profile.data_analyst")}
                                </p>
                            </div>
            ...
            <div className="container mx-auto px-4 py-8">
                <Tabs defaultValue="showcase" className="space-y-8">
                    <TabsList>
                        <TabsTrigger value="showcase">{t("public_profile.showcase")}</TabsTrigger>
                        <TabsTrigger value="projects">{t("public_profile.track_record")}</TabsTrigger>
                        <TabsTrigger value="reviews">{t("public_profile.reviews")}</TabsTrigger>
                    </TabsList>
                        ...
                        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                            {profile.dashboards && profile.dashboards.length > 0 ? (
                                            ...
                                            <CardHeader className="pb-2">
                                                <CardTitle className="text-lg flex items-center justify-between">
                                                    ...
                                                    <Badge variant="outline" className="border-yellow-500 text-yellow-600 bg-yellow-50 text-xs font-normal">
                                                        <Star className="h-3 w-3 mr-1 rtl:ml-1 rtl:mr-0 fill-yellow-500" /> {t("public_profile.featured")}
                                                    </Badge>
                                                </CardTitle>
                                            ...
                                            <CardContent className="mt-auto pt-4">
                                                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        {dashboard.layout?.items?.length || 0} {t("public_profile.charts")}
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                ...
                                <div className="col-span-full flex flex-col items-center justify-center py-12 text-muted-foreground bg-muted/20 rounded-lg border border-dashed">
                                    ...
                                    <p>{t("public_profile.no_showcase")}</p>
                                </div>
                            )}
                        </div>
                    </TabsContent>
                        ...
                        <div className="grid gap-4">
                            {profile.projects && profile.projects.length > 0 ? (
                                ...
                                <div className="text-center py-12 text-muted-foreground">
                                    <p>{t("public_profile.no_projects")}</p>
                                </div>
                            )}
                        </div>
                    </TabsContent>
                    ...
                    <TabsContent value="reviews">
                        <div className="space-y-4">
                            {profile.ratings && profile.ratings.length > 0 ? (
                                profile.ratings.map((rating: any) => (
                                    <Card key={rating.id}>
                                        <CardContent className="pt-6">
                                            ...
                                            <p className="text-sm italic">"{rating.comment || t("public_profile.no_comment")}"</p>
                                            <div className="mt-4 text-xs text-muted-foreground flex items-center gap-2">
                                                <Award className="h-3 w-3" />
                                                {t("public_profile.verified_review")}
                                ...
                                <div className="text-center py-12 text-muted-foreground">
                                    <p>{t("public_profile.no_reviews")}</p>
                                </div>
                            )}
                        </div>
                    </TabsContent>
